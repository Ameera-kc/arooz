        ...
        try:
            ...
            for i in cart_obj:
                data1 = {
                    ...
                    'sub_total': i.total, 
                    'rent_security':i.product.rentalsecurity,
                    'amount':amount
                }
                data.append(data1)
               
            for i in data:
                messagestring +="%0aProduct-Name:"+str(i['name'])+"%0aQuantity:"+str(i['quantity'])+"%0aUnit-Price:"+str(i['price'])+"%0aTotal :"+str(i['sub_total'])+"%0aRent-Security :"+str(i['rent_security'])+"%0aAmount-To-Pay :"+str(i['amount'])+"%0a-----------------------------"
                
            messagestring+="%0a-----------------------------%0a\
            Grand Total :"+str(amount)+"%0a-----------------------------"
            ...
            if Measurements.objects.filter(user=customer):
                ...
                for i in measurement.values():
                    print(i['hip'],"measurement")
                    messagestring+="%0aSleeve-length:"+str(i['sleevelength'])+"%0aChest-around:"+str((i['chestaround']))+"%0aWaist-Around:"+str((i['waistAround']))+"%0a-----------------------------"
                    messagestring+="%0aLength-Pant:"+str(i['LengthTrouser'])+"%0aHip:"+str(i['hip'])+"%0aWaist-For-Trouser:"+str(i['WaistForTrouser'])+"%0aThighs:"+str(i['thighs'])+"%0aHem:"+str(i['hem'])
                messagestring+="%0a-----------------------------%0a"
            else:
                messagestring+="%0a-----------------------------%0a"
            if Size.objects.filter(user=customer):
...
def measurements(request,id):
    ...
    if request.user.is_authenticated:
        if Customer.objects.get(user = request.user):
            ...
            Measurements.objects.create(user=customer,product=product,sleevelength=sleevelength,chestaround=chestaround,waistAround=waistAround,LengthTrouser=Length,hip=Hip,WaistForTrouser=Waist,thighs=Thighs,hem=Hem)
